	...
	public class SimplexNoise
	{
		...
		public static float octave_noise_3d( float octaves, float persistence, float scale, float x, float y, float z ) {
		    ...
		    for( int i=0; i < octaves; i++ ) {
		        total += (float)noise( x * frequency, y * frequency, z * frequency ) * amplitude;
		...
		public static double F22d = 0.5 * (Math.Sqrt(3.0) - 1.0);
		public static double G22d = (3.0 - Math.Sqrt(3.0)) / 6.0;
	    public static double noise(double xin, double yin) {
	        double n0, n1, n2;
			// Noise contributions from the three corners
			// Skew the input space to determine which simplex cell we're in
	        //static const double F2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
	        double s = (xin + yin) * F22d;
			// Hairy factor for 2D
	        int i = fastfloor(xin + s);
	        int j = fastfloor(yin + s);
	        //static const double G2 = (3.0 - Math.Sqrt(3.0)) / 6.0;
	        double t = (i + j) * G22d;
	        double X0 = i - t;
			...
			// c = (3-sqrt(3))/6
	        double x1 = x0 - i1 + G22d;
			// Offsets for middle corner in (x,y) unskewed coords
	        double y1 = y0 - j1 + G22d;
	        double x2 = x0 - 1.0 + 2.0 * G22d;
			// Offsets for last corner in (x,y) unskewed coords
	        double y2 = y0 - 1.0 + 2.0 * G22d;
		...
		public static double F3 = 1.0 / 3.0;
		public static double G3 = 1.0 / 6.0;
